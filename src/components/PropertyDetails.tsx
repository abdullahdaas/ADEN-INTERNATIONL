    ...
    <div id="details-view" className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <button
          id="btn-details-back"
          onClick={onBack}
          className="flex items-center space-x-1.5 space-x-reverse rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-slate-300 hover:text-white hover:bg-white/10 transition-all cursor-pointer"
        >
        ...
        <div className="flex items-center space-x-2 space-x-reverse">
          {user && (
            <button
              ...
              className="flex items-center space-x-1.5 space-x-reverse rounded-xl border border-white/10 bg-[#F27D26] px-4 py-2 text-sm font-bold text-white hover:bg-[#d96a1a] transition-all shadow-lg shadow-[#F27D26]/20"
            >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="relative rounded-2xl border border-white/5 bg-slate-950 overflow-hidden group shadow-2xl">
            ...
            <div className="relative h-96 w-full cursor-zoom-in" onClick={() => setLightboxOpen(true)}>
              ...
              <div className="absolute bottom-4 right-4 rounded-lg bg-black/60 px-3 py-1.5 font-mono text-xs font-bold text-white backdrop-blur-md">
          ...
          <div className="rounded-2xl border border-white/5 bg-slate-900/10 backdrop-blur-md p-6 space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="rounded bg-gold-prestige/10 border border-gold-prestige/25 text-gold-prestige px-2.5 py-0.5 text-xs font-black">
                    {property.status}
                  </span>
                  ...
                  {property.isFeatured && (
                    <span className="text-xs bg-yellow-500/10 text-yellow-400 border border-yellow-500/20 px-2 py-0.5 rounded font-bold animate-pulse">
                      ⭐ إعلان ترقية مستمرة
                    </span>
                  )}
                </div>
            ...
            {property.isAuction && property.isAuctionActive && (
              <div className="mt-4 p-4 rounded-xl border border-amber-500/30 bg-amber-500/10 flex flex-col md:flex-row items-center justify-between gap-4 animate-fade-in">
                ...
                <button onClick={() => window.alert('سيتم توفير خيار المشاركة في المزاد للمسجلين.')} className="px-6 py-2.5 rounded-lg bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold transition-all shadow-lg shadow-amber-600/20">
            ...
            {property.documents && property.documents.length > 0 && (
              <div className="mt-4 p-4 rounded-xl border border-white/5 bg-slate-900/40">
                <h3 className="text-white font-bold text-xs mb-3 border-b border-white/5 pb-2">مستندات العقار</h3>
                <div className="flex flex-wrap gap-2">
                  {property.documents?.filter(d => d.isPublic || user?.role === 'admin' || user?.emailOrPhone === property.ownerEmailOrPhone)?.map((doc, i) => (
                    <a key={i} href={doc.url} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-white/10 bg-slate-950 text-[10px] text-slate-300 hover:text-white hover:border-[#F27D26]/40 transition-all">
                      📄 {doc.title}
                    </a>
                  ))}
                  {property.documents.filter(d => d.isPublic || user?.role === 'admin' || user?.emailOrPhone === property.ownerEmailOrPhone).length === 0 && (
                    <span className="text-[10px] text-slate-500">لا توجد مستندات عامة متاحة.</span>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="flex items-center space-x-1.5 space-x-reverse text-sm text-slate-300">
            ...
            <div className="grid grid-cols-3 gap-4 border-t border-white/5 pt-4 text-xs text-slate-400 font-sans">
            ...
            <div className="rounded-2xl border border-gold-prestige/10 bg-slate-950 p-6 space-y-4 shadow-xl">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center space-x-3 space-x-reverse">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 font-sans mt-0.5">قم بترقية إعلانك ليظهر في مقدمة نتائج البحث ومعامل الإبرام الفوري</p>
                  </div>
                </div>
                <button
                  id="btn-open-upgrade-panel"
                  onClick={() => setUpgradeOpen(!upgradeOpen)}
                  className="rounded-xl bg-gold-prestige px-5 py-2.5 text-xs font-bold text-white hover:bg-gold-accent transition-all cursor-pointer shrink-0"
                >
              ...
              {upgradeOpen && (
                <div className="border-t border-white/10 pt-5 space-y-5 animate-slide-down">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-300 mb-3">أولاً: اختر الباقة الترويجية المناسبة:</h4>
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 p-4 rounded-xl bg-slate-900/50 border border-white/5 text-xs leading-relaxed">
                  ...
                  <form onSubmit={handlePaymentSubmit} className="space-y-4">
                    <h4 className="text-xs font-bold text-slate-300">ثانياً: أرسل تفاصيل التحويل لإدارة المنصة ليتم تفعيل الترقية مباشرة:</h4>
                    
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                      <div>
                        <label className="block text-[11px] text-slate-400 mb-1">طريقة الدفع المستخدمة</label>
                        <select
                          value={paymentMethod}
                          onChange={(e: any) => setPaymentMethod(e.target.value)}
                          className="w-full rounded-lg border border-white/5 bg-slate-900 px-3 py-2 text-xs text-white"
                        >
                      ...
                      <div>
                        <label className="block text-[11px] text-slate-400 mb-1">رقم الهاتف للتحقق</label>
                        <input
                          type="text"
                          required
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <div>
                        <label className="block text-[11px] text-slate-400 mb-1">رقم المعاملة / إشعار التحويل (اختياري)</label>
                        <input
                          type="text"
                          placeholder="أدخل رمز العملية المكون من أرقام"
                          value={transactionId}
                          onChange={(e) => setTransactionId(e.target.value)}
                          className="w-full rounded-lg border border-white/5 bg-slate-900 px-3 py-2 text-xs text-white placeholder-slate-600 outline-none font-mono"
                        />
                      </div>
                      ...
                      <div className="flex items-end">
                        <button
                          ...
                          type="submit"
                          className="w-full rounded-lg bg-emerald-600 px-4 py-2 text-xs font-bold text-white hover:bg-emerald-500 transition-all cursor-pointer flex items-center justify-center gap-1.5"
                        >
                    ...
                    {paymentSubmitted && (
                      <div className="rounded-lg bg-emerald-500/10 p-3.5 border border-emerald-500/20 text-xs text-emerald-400 flex items-center gap-2">
          ...
          <div className="rounded-2xl border border-white/5 bg-slate-900/10 backdrop-blur-md p-6 space-y-6">
            ...
            <form onSubmit={handleReviewSubmit} className="space-y-4 pt-4 border-t border-white/5">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-white/5 bg-slate-950 px-3 py-2 text-xs text-white placeholder-slate-600 outline-none"
                  />
                </div>
                <div>
                  <label className="block text-[11px] text-slate-400 mb-1">التقييم العام بالنجوم</label>
              ...
              <div>
                <label className="block text-[11px] text-slate-400 mb-1">تفاصيل التعليق والملاحظات</label>
                <textarea
                  rows={3}
                  required
                  ...
                  className="w-full rounded-lg border border-white/5 bg-slate-950 px-3 py-2 text-xs text-white placeholder-slate-600 outline-none font-sans"
                />
              </div>
              ...
              <div className="flex justify-end">
                <button
                  ...
                  type="submit"
                  className="rounded-lg bg-gold-prestige hover:bg-gold-accent px-5 py-2 text-xs font-bold text-white transition-all cursor-pointer"
                >
                  نشر المراجعة الآن
                </button>
              </div>
              ...
              {reviewSubmitted && (
                <div className="rounded-lg bg-emerald-500/10 p-3.5 border border-emerald-500/20 text-xs text-emerald-400 text-center">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-white/5 bg-slate-900/10 backdrop-blur-md p-6 space-y-6">
            ...
            <div className="flex items-center space-x-3 space-x-reverse">
              ...
              <div>
                <h4 className="text-sm font-extrabold text-white flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] text-gold-prestige font-bold bg-gold-prestige/10 px-1.5 py-0.5 rounded">مدير معتمد</span>
                </h4>
                <p className="text-xs text-slate-400 font-sans mt-0.5">عدن للوساطة العقارية</p>
            ...
            <p className="text-xs text-slate-300 leading-relaxed font-sans border-t border-b border-white/5 py-3">
            ...
            <div className="space-y-3.5 pt-2">
              <a
                ...
                className="w-full flex items-center justify-center space-x-2 space-x-reverse rounded-xl bg-gold-prestige hover:bg-gold-accent py-3 text-xs font-bold text-white transition-all shadow-lg shadow-gold-prestige/10"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="w-full flex items-center justify-center space-x-2 space-x-reverse rounded-xl bg-emerald-600 hover:bg-emerald-500 py-3 text-xs font-bold text-white transition-all shadow-lg"
              >
              ...
              {agent.telegram && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="w-full flex items-center justify-center space-x-2 space-x-reverse rounded-xl bg-sky-600 hover:bg-sky-500 py-3 text-xs font-bold text-white transition-all"
                >
                  <Send className="h-4 w-4" />
              ...
              <a
                ...
                className="w-full flex items-center justify-center space-x-2 space-x-reverse rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 py-3 text-xs font-semibold text-slate-300 transition-all"
              >
              ...
              <button
                ...
                className="w-full flex items-center justify-center space-x-2 space-x-reverse rounded-xl border border-emerald-500/20 bg-emerald-500/10 hover:bg-emerald-500/20 py-3 text-xs font-semibold text-emerald-400 transition-all mt-2"
              >
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
              <div className="bg-slate-900 border border-emerald-500/30 rounded-2xl p-6 w-full max-w-md space-y-4 shadow-2xl">
                ...
                <div className="space-y-3">
                  <div>
                    <label className="block text-xs text-slate-300 mb-1">تاريخ الزيارة</label>
                    <input
                      type="date"
                      value={visitDate}
                  ...
                  <div>
                    <label className="block text-xs text-slate-300 mb-1">الوقت المقترح</label>
                    <input
                      type="time"
                      value={visitTime}
                ...
                <div className="flex gap-2 pt-4">
                  <button onClick={handleBookVisit} className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white py-2 rounded-lg text-xs font-bold transition-all">
                    تأكيد حجز الموعد
                  </button>
                  <button onClick={() => setShowVisitModal(false)} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-xs font-bold transition-all">
                    إلغاء
                  </button>
                </div>
              </div>
            ...
            <div className="rounded-2xl border border-[#F27D26]/20 bg-gradient-to-b from-[#F27D26]/10 to-[#050505]/30 backdrop-blur-md p-6 space-y-4 shadow-xl">
              <div className="flex items-center space-x-2 space-x-reverse">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[#F27D26] font-sans">تواصل مباشر بدون عمولات أو رسوم وساطة</p>
                </div>
              </div>
              
              <div className="border-t border-white/5 pt-3 space-y-3">
                <div className="flex justify-between items-center bg-white/5 p-2.5 rounded-lg border border-white/5">
                  <span className="text-xs text-slate-400">اسم المعلن / المالك:</span>
                  <span className="text-xs font-bold text-white">{property.advertiserName || 'صاحب الإعلان'}</span>
                </div>
                ...
                {property.ownerEmailOrPhone && onViewProfile && (
                  <button
                    ...
                    className="w-full flex items-center justify-center space-x-2 space-x-reverse rounded-xl bg-white/5 border border-white/5 hover:bg-[#F27D26]/10 hover:border-[#F27D26]/30 hover:text-white py-3 text-xs font-bold text-slate-300 transition-all cursor-pointer"
                  >
                ...
                <a
                  ...
                  className="w-full flex items-center justify-center space-x-2 space-x-reverse rounded-xl bg-gradient-to-r from-[#F27D26] to-[#ff8a3d] py-3 text-xs font-bold text-white hover:scale-[1.02] active:scale-[0.98] transition-all shadow-lg"
                >
                  <Phone className="h-4 w-4" />
                ...
                {property.advertiserWhatsapp && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="w-full flex items-center justify-center space-x-2 space-x-reverse rounded-xl bg-emerald-600 hover:bg-emerald-500 py-3 text-xs font-bold text-white hover:scale-[1.02] active:scale-[0.98] transition-all shadow-lg"
                  >
                    <MessageCircle className="h-4.5 w-4.5" />
          ...
          <div className="rounded-2xl border border-white/5 bg-slate-900/10 backdrop-blur-md p-6 space-y-4">
            ...
            {nearbyProps.length > 0 && (
              <div className="mt-4 pt-4 border-t border-white/5 space-y-3">
                <h4 className="text-xs font-bold text-white mb-2">عقارات قريبة منك</h4>
                {nearbyProps?.map(np => (
                  <button 
                    key={np.id} 
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-start">
                        <span className="text-[10px] font-bold text-white line-clamp-1">{np.title}</span>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="w-full flex items-center justify-center space-x-2 space-x-reverse rounded-xl border border-gold-prestige/30 bg-gold-prestige/10 hover:bg-gold-prestige/20 py-2.5 text-xs font-bold text-gold-prestige transition-all text-center"
            >
                ...
                {nearbyProps?.map((prop) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <span className="text-[11px] text-gold-prestige font-bold block font-sans mt-0.5">
      ...
      {lightboxOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 p-4 animate-fade-in">
          ...
          <div className="relative max-h-[85vh] max-w-5xl overflow-hidden rounded-xl border border-white/10 bg-black">
            <img 
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full bg-black/70 border border-white/10 px-4 py-1.5 font-mono text-xs font-bold text-white">
