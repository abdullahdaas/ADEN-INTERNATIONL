...
const regex = /app\.post\('\/api\/citizen-login', async \(req, res\) => \{[\s\S]*?(?=app\.post\('\/api\/login',)/;
...
console.log("Regex replace finished. Output match: ", regex.test(fs.readFileSync('server.ts', 'utf8')));
